...
func Register(openAPI *gin.RouterGroup) {
	...
	mid.Sub("openapi").Reg(openAPI, "/knowledge", http.MethodPost, openapi.CreateKnowledge, "新建知识库", middleware.AuthOpenAPIKey(constant.OpenAPITypeKnowledge))
	...
	mid.Sub("openapi").Reg(openAPI, "/knowledge/doc/list", http.MethodGet, openapi.GetDocList, "获取文档列表", middleware.AuthOpenAPIKey(constant.OpenAPITypeKnowledge), middleware.AuthKnowledge("knowledgeId", middleware.KnowledgeView))
	...
	mid.Sub("openapi").Reg(openAPI, "/knowledge/hit", http.MethodPost, openapi.KnowledgeHit, "知识库命中测试", middleware.AuthOpenAPIKey(constant.OpenAPITypeKnowledge))
