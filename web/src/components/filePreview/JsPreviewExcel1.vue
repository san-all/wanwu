...
const initPreviewer = () => {
  myExcelPreviewer.value = jsPreviewExcel.init(
    document.getElementById('comac-preview-excel'),
  );
  ...
  myExcelPreviewer.value.setOptions({
    transformData: workbookData => {
      ...
      dataIndex = workbookData.findIndex(
        item => item.name === page.value.sheetName,
      );
      if (
        ...
        if (row.cells) {
          for (let key in cells) {
            //高亮RowNum
            workbookData[dataIndex].styles[cells[key].style].bgcolor =
              '#ff0000';
